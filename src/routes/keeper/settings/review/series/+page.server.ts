...
import { approveProposed, listProposedSeries, rejectProposed } from '$lib/server/db/queries';
...
export const load: PageServerLoad = async ({ locals }) => {
	...
	return {
		...
		series: await listProposedSeries()
	};
};
...
export const actions: Actions = {
	approve: async ({ request, locals }) => {
		...
		if (!id.success) return fail(400, { reviewError: 'Unknown series' });
		await approveProposed('series', id.data, locals.user.id);
	...
	reject: async ({ request, locals }) => {
		...
		const id = idSchema.safeParse((await request.formData()).get('id'));
		if (!id.success) return fail(400, { reviewError: 'Unknown series' });
		await rejectProposed('series', id.data, locals.user.id);
